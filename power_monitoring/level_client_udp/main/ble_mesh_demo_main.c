...
void send_message_BLE(uint16_t addr, uint32_t opcode, int16_t level, bool send_rel) {
    ...
    if (err) {
        ...
        ESP_LOGI("PC-BLE", "[status: S, level: %d ]", level);
    }
}
    ...
    switch (event) {
        ...
        case ESP_BLE_MESH_GENERIC_CLIENT_SET_STATE_EVT: {
            ...
            if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_LEVEL_SET) {
                ESP_LOGI("PC-BLE", "[opcode: R, level: %d ttl: %d]", param->status_cb.level_status.present_level,
                         param->params->ctx.recv_ttl);
            } else if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_LEVEL_SET_UNACK) {
                ESP_LOGI("PC-BLE", "[opcode: E, level: %d ttl: %d]", my_info_level, 0);
            }
            //ESP_LOGI(TAG_BLE, "--- SET_STATE_EVT 0x%x", param->params->opcode);
            break;
        }
        case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT: {
            ESP_LOGI("PC-BLE", "[opcode: P, level: %d ttl: %d]", param->status_cb.level_status.present_level,
                     param->params->ctx.recv_ttl);
            break;
        }
...
//             param->error_code, param->params->ctx.addr, param->params->opcode);
}
    ...
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT) {
        switch (param->ctx.recv_op) {
            ...
            case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
                ...
                // init and start RULE BLE and WIFI
