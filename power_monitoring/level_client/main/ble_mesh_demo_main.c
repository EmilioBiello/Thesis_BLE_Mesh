    ...
    switch (event) {
        ...
        case ESP_BLE_MESH_GENERIC_CLIENT_SET_STATE_EVT: {
            ...
            if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_LEVEL_SET) {
                ESP_LOGI("PC", "[opcode: R, level: %d ttl: %d]", param->status_cb.level_status.present_level,
                         param->params->ctx.recv_ttl);
            } else if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_LEVEL_SET_UNACK) {
                ESP_LOGI("PC", "[opcode: E, level: %d ttl: %d]", my_info_level, 0);
            }
            ESP_LOGI(TAG, "--- SET_STATE_EVT 0x%x", param->params->opcode);
            break;
        }
        case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT: {
            ESP_LOGI("PC", "[opcode: P, level: %d ttl: %d]", param->status_cb.level_status.present_level,
                     param->params->ctx.recv_ttl);
            break;
        }
