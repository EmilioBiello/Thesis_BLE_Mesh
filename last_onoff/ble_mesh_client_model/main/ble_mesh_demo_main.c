...
void print_message(esp_ble_mesh_client_common_param_t common, esp_ble_mesh_generic_client_set_state_t set_default) {
    printf("Message:\n");
    printf("- Contex: NetKey index of the subnet \t %hu\n", common.ctx.net_idx);
    printf("- Contex: AppKey Index for message encryption \t %hu\n", common.ctx.app_idx);
    printf("- Contex: Remote Address \t %hu\n", common.ctx.addr);
    //printf("- Contex: Destination Address \t %hu\n", common.ctx.recv_dst); // Not Used for sending
    //printf("- Contex: Received TTL \t%hhu\n", common.ctx.recv_ttl); // Not Used for sending
    printf("- Contex: TTL \t %hhu\n", common.ctx.send_ttl);
    //printf("- Contex: Opcode of a received message \t %u\n", common.ctx.recv_op);// Not Used for sending
    esp_ble_mesh_gen_onoff_set_t set = set_default.onoff_set;
    printf("* SET: Target value of Generic OnOff state %hhu\n", set.onoff);
    printf("* SET: Transaction ID %hhu\n", set.tid);
}
