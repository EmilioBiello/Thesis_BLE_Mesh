...
#include "include/ble_mesh_init.h"
/* BLE */
...
void mesh_scan_done_handler(int num) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (ie_len == sizeof(assoc)) {
            ESP_LOGW(MESH_TAG,
                     "<MESH>[%d]%s, layer:%d/%d, assoc:%d/%d, %d, "
                             MACSTR
                             ", channel:%u, rssi:%d, ID<"
                             MACSTR
                             "><%s>",
                     ...
                     assoc.encrypted ? "IE Encrypted" : "IE Unencrypted");
    ...
    if (parent_found) {
        ...
        if (my_type == MESH_ROOT) {
            ...
            ESP_LOGW(MESH_TAG,
                     "<PARENT>%s, layer:%d/%d, assoc:%d/%d, %d, "
                             MACSTR
                             ", channel:%u, rssi:%d",
...
void esp_mesh_p2p_rx_main(void *arg) {
    ...
    while (is_running) {
        ...
        board_led_operation_wifi(status_led_wifi);
...
void mesh_event_handler(void *arg, esp_event_base_t event_base,
    ...
    switch (event_id) {
        case MESH_EVENT_STARTED: {
            esp_mesh_get_id(&id);
            ESP_LOGI(MESH_TAG, "<MESH_EVENT_STARTED>ID:"
                    MACSTR
                    "", MAC2STR(id.addr));
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index) {
    ...
    board_led_operation_wifi(LED_OFF);
...
static esp_err_t ble_mesh_init(void) {
    ...
    board_led_operation_wifi(LED_ON);
