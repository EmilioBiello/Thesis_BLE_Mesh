...
esp_err_t bluetooth_init(void)
{
    ...
    if (ret) {
        ESP_LOGE(TAG_BLE, "%s init bluetooth failed", __func__);
        return ret;
    }
    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(TAG_BLE, "%s enable bluetooth failed", __func__);
        return ret;
    }
...
static void mesh_on_sync(void)
{
    ...
    if (rc != 0) {
        ESP_LOGI(TAG_BLE, "error determining address type; rc=%d", rc);
        return;
    }
...
esp_err_t bluetooth_init(void)
{
    ...
    if (mesh_sem == NULL) {
        ESP_LOGE(TAG_BLE, "Failed to create mesh semaphore");
        return ESP_FAIL;
    }
